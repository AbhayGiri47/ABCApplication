...
class CatalogAnalysisBottomSheet : BottomSheetDialogFragment() {
    ...
    private fun onDataReceived(data: CatalogAnalysis) {
        with(binding) {
            tvItemCount.text = getString(R.string.txt_item_count,data.catalogType, data.itemCount)
            val sb = StringBuilder()
            data.characterOccurrences.map { character->
                sb.append(getString(R.string.list_character_count,character.key,character.value.toString()))
            }
            tvContent1.text = sb
        }
    }
